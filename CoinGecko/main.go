...
func mockCryptoData(coinID string) CryptoPrice {
	...
	case "solana":
		return CryptoPrice{
			...
			CurrentPrice: applyRandomVariation(153.0, 1),
			AllTimeHigh:  250.0,
			AllTimeLow:   10.0,
	...
	case "ethereum":
		return CryptoPrice{
			...
			CurrentPrice: applyRandomVariation(2655.02, 1),
			AllTimeHigh:  4800.0,
			AllTimeLow:   100.0,
			UpdatedAt:    time.Now().Format(time.RFC3339),
		}
	default:
		return CryptoPrice{
			...
			CurrentPrice: applyRandomVariation(99000.0+float64(len(coinID))*100, 1),
